...
class Find(PlexManager, help='Find Plex information'):
    ...
    query = PassThru(help=f'Query in the format --field[__operation] value; valid operations: {OPS}')
...
class Rate(PlexManager, help='Update ratings in Plex'):
    ...
    def main(self, *args, **kwargs):
        ...
        find_and_rate(self.plex, self.rating, self.obj_type, self.title, dynamic, self.escape, self.allow_inst)
