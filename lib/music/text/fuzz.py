...
def fuzz_process(text, strip_special=True, space=True):
    ...
    if not text:
        return text
    try:
        non_letter_non_num_sub = fuzz_process._non_letter_non_num_sub
        ost_sub = fuzz_process._ost_sub
    except AttributeError:
        import re
        non_letter_non_num_sub = fuzz_process._non_letter_non_num_sub = re.compile(r'\W').sub
        ost_sub = fuzz_process._ost_sub = re.compile(r'\sOST(?:$|\s|\)|\])', re.IGNORECASE).sub

    original = text
    if strip_special:                               # Some titles are only differentiable by special characters
        text = non_letter_non_num_sub(' ', text)    # Convert non-letter/numeric characters to spaces
    sp = ' ' if space else ''
    text = sp.join(text.split())                    # Condense sets of consecutive spaces to 1 space (faster than regex)
    text = ost_sub('', text)                        # Remove 'OST' to prevent false positives based only on that
