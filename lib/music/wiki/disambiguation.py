...
from wiki_nodes import MediaWikiClient, WikiPage, Link, CompoundNode, List, Section
...
def handle_disambiguation_candidates(
    ...
    :param client: The wiki http client for the disambiguation page being processed
    :param candidates: Mapping of {Link: (WikiEntity subclass, WikiPage)}
    :param existing: An existing WikiEntity representing the same actual entity as the intended page that should be
      found; used to filter links/candidates.
    :param name: The Name that was searched for
    :param prompt: Prompt to interactively resolve ambiguous candidates after filtering
    ...
    if len(candidates) == 1:
        ...
        matches = _filtered_candidates(existing.name, candidates, existing)  # noqa
        ...
        if _should_skip(p_name, links):  # noqa
            choice = '[None - skip]'
        else:
...
def _filtered_candidates(name: Name, candidates: Candidates, existing: WE = None) -> Candidates:
    matches = {}
...
def _disambiguation_entries(list_node: List) -> Iterator[Link]:
